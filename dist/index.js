...
var DEPENDENCY_FIELDS = ["dependencies", "devDependencies", "optionalDependencies", "peerDependencies"];
async function analyzeRepository(ctx) {
  ...
  const contextSources = needsRepositoryContext ? await loadDependencyMetadataContext(ctx.repoPath, sources) : sources;
  const detections = spec.rules.flatMap((rule) => rule.match.kind === "direct-dependency-drift" ? findDirectDependencyDrift(rule, contextSources) : evaluate(rule, sources, allPaths));
...
function findDirectDependencyDrift(rule, sources) {
  ...
  const shrinkwrapRoots = new Set(candidates.filter((source) => source.path.endsWith("npm-shrinkwrap.json")).map((source) => dirname2(source.path)));
  const lockfiles = candidates.filter((source) => source.path.endsWith("npm-shrinkwrap.json") || !shrinkwrapRoots.has(dirname2(source.path)));
  for (const lockfile of lockfiles) {
    const root = dirname2(lockfile.path);
    let lock;
    try {
    ...
    for (const [lockKey, rawLockPackage] of Object.entries(packages)) {
      ...
      for (const field of DEPENDENCY_FIELDS) {
        ...
        const names = [.../* @__PURE__ */ new Set([...Object.keys(manifestDependencies), ...Object.keys(lockedDependencies)])].sort();
        for (const name of names) {
          const declared = manifestDependencies[name];
          const locked = lockedDependencies[name];
          if (declared === locked) continue;
          const change = declared === void 0 ? "removed" : locked === void 0 ? "missing" : "changed";
          const index = declared === void 0 ? locateLockPackageKey(lockfile.source, lockKey, field, name) : locateJsonKey(manifestSource.source, name, field);
          const evidenceSource = declared === void 0 ? lockfile : manifestSource;
          detections.push({
            rule,
...
function locateEligible(file, expression, anchors) {
  ...
  for (const match of file.source.matchAll(new RegExp(expression.pattern, flags))) {
    ...
    const line = file.status === "modified" && anchors !== void 0 ? eligibleSemanticAnchor(file, match.index, match[0], anchors) : eligibleRangeAnchor(file, match.index, match.index + match[0].length);
    if (line !== void 0) return locateFromLine(file.source, line);
  }
  return void 0;
}
...
// src/index.ts
function createApp() {
  const app = new Adversary({ name: "npm", version: "0.0.13", review: { maximumFindings: 8 } });
  registerRules(app);
  app.rule("npm.review", async (ctx) => analyzeRepository(ctx));
  return app;
}
